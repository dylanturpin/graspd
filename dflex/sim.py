...
def box_sdf_grad(upper: df.float3, p: df.float3):
    ...
    abs_p = df.float3(abs(p[0]),abs(p[1]),abs(p[2]))

    if (abs_p[0] > abs_p[1] and abs_p[0] > abs_p[2]):
        return df.float3(df.sign(p[0]), 0.0, 0.0)

    if (abs_p[1] > abs_p[0] and abs_p[1] > abs_p[2]):
        return df.float3(0.0, df.sign(p[1]), 0.0)

    return df.float3(0.0, 0.0, df.sign(p[2]))
...
def eval_rigid_contacts_art(
    ...
    shape_geo_type: df.tensor(int), 
    shape_geo_scale: df.tensor(df.float3),
    geo_sdf: df.tensor(float),
    contact_body0: df.tensor(int),
    contact_body1: df.tensor(int),
    contact_shape0: df.tensor(int),
    contact_shape1: df.tensor(int),
    contact_point: df.tensor(df.float3),
    contact_dist: df.tensor(float),
    contact_mat: df.tensor(int),
    materials: df.tensor(float),
    ...
    c_body0 = df.load(contact_body0, tid)
    c_body1 = df.load(contact_body1, tid)
    c_shape0 = df.load(contact_shape0, tid)
    c_shape1 = df.load(contact_shape1, tid)
    c_point = df.load(contact_point, tid)
    c_dist = df.load(contact_dist, tid)
    c_mat = df.load(contact_mat, tid)

    # handle contact with ground
    if (c_body1 == (0-1)): # workaround for unary op
        # hard coded surface parameter tensor layout (ke, kd, kf, mu)
        ke = df.load(materials, c_mat * 4 + 0)       # restitution coefficient
        kd = df.load(materials, c_mat * 4 + 1)       # damping coefficient
        kf = df.load(materials, c_mat * 4 + 2)       # friction coefficient
        ...
        X_s = df.load(body_X_s, c_body0)              # position of colliding body
        v_s = df.load(body_v_s, c_body0)              # orientation of colliding body

        n = float3(0.0, 1.0, 0.0)

        # transform point to world space
        p = df.spatial_transform_point(X_s, c_point)
        ...
        # check ground contact
        c = df.min(dot(n, p) - c_dist, 0.0)         # check if we're inside the ground
        ...
        f_total = n * (fn + fd) + ft
        t_total = df.cross(c_point, f_total)

        df.atomic_add(body_f_s, c_body0, df.spatial_vector(t_total, f_total))
        return
    # handle shape to shape contact
    #if (c_body1 >= 0): # (else not yet supported)
    # use contact point from body0 to query sdf for body1
    ...
    r0 = p - df.spatial_transform_get_translation(X_s)
    dpdt = v + df.cross(w, r0)

    # =====================  TODO REMOVE THIS=====
    # rename vars to match eval_soft_contact
    px = p
    pv = dpdt

    X_sc = df.load(body_X_s, c_body1)

    X_co = df.load(shape_X_co, c_shape1)
    ...
    # get normal of contact point on shape 0
    geo_type0 = df.load(shape_geo_type, c_shape0)
    geo_scale0 = df.load(shape_geo_scale, c_shape0)
    n0 = df.float3(0.0, 0.0, 0.0)
    # if sphere, just agree with n1
    # box
    if (geo_type0 == 1):
        grad = box_sdf_grad(geo_scale0, c_point)
        n0 = df.spatial_transform_vector(df.spatial_transform_inverse(X_s), grad)

    # geo description
    geo_type = df.load(shape_geo_type, c_shape1)
    geo_scale = df.load(shape_geo_scale, c_shape1)
    ...
    # GEO_SPHERE (0)
    if (geo_type == 0):
        d = sphere_sdf(df.float3(0.0, 0.0, 0.0),geo_scale[0], x_local)-margin
        # a = kd
        # if a == 0.0:
        #     c = df.min(d, 0.0)
        # if a != 0.0:
    ...
    # GEO_BOX (1)
    if (geo_type == 1):
        d = box_sdf(geo_scale, x_local) - margin
        # a = kd
        # if a == 0.0:
        #     c = df.min(d, 0.0)
        # if a != 0.0:
    ...
    # GEO_CAPSULE (2)
    if (geo_type == 2):
        c = df.leaky_min(capsule_sdf(geo_scale[0], geo_scale[1], x_local)-margin, 0.0, 0.1)
        n = df.spatial_transform_vector(X_so, capsule_sdf_grad(geo_scale[0], geo_scale[1], x_local))
    ...
    if (geo_type == 4):
        ...
        eps = 1e-5
        x0 = df.index(x_local,0)
        x1 = df.index(x_local,1)
        x2 = df.index(x_local,2)
        n_ = df.float3(
            (get_sdf(geo_scale, df.float3(x0+eps,x1,x2), geo_sdf,tid) - get_sdf(geo_scale, df.float3(x0-eps,x1,x2), geo_sdf,tid)) / (2.0*eps),
            (get_sdf(geo_scale, df.float3(x0,x1+eps,x2), geo_sdf,tid) - get_sdf(geo_scale, df.float3(x0,x1-eps,x2), geo_sdf,tid)) / (2.0*eps),
            (get_sdf(geo_scale, df.float3(x0,x1,x2+eps), geo_sdf,tid) - get_sdf(geo_scale, df.float3(x0,x1,x2-eps), geo_sdf,tid)) / (2.0*eps)
        )
        n = df.spatial_transform_vector(X_so, df.normalize(n_))

    # if shape0 is sphere, match normals
    if (geo_type0 == 0):
        n0 = n * (0.0 - 1.0)

    # scale c by scalar projection of n0 onto n
    # n0_proj_n = df.dot(n0,n) * (0.0-1.0)# no it does get a sign uhh.
    # n0_proj_n = df.max(0.0, n0_proj_n)
    # c = n0_proj_n * c
    ...
    # rigid velocity
    rigid_v_s = df.spatial_vector()   
    rigid_v_s = df.load(body_v_s, c_body1)
    ...
    # Coulomb friction (box)
    lower = mu * (c * ke)# + df.min(vn, 0.0) * kd) # = mu * (|fn| + |fd|)
    upper = 0.0 - lower      # workaround because no unary ops yet
    ...
    #vx = df.clamp(df.dot(nx * kf * (0.0-c), vt), lower, upper)
    #vz = df.clamp(df.dot(nz * kf * (0.0-c), vt), lower, upper)
    ...
    # CAN BORROW CODE for normal directions from rigid collision triangles above!!
    ...
    # moment arm, spatial transform point as vector from shape to world
    # r = p - x0
    #r0 = df.spatial_transform_vector(X_s, c_point)
...
def sub2ind(
    ...
    #return x * size_y * size_z + y* size_z + z # row major
    return x + y * size_x + z * size_y * size_x # col major
...
def get_sdf(
    ...
    x = df.index(p_local, 0)
    y = df.index(p_local, 1)
    z = df.index(p_local, 2)

    # if query point not in volume, use sphere sdf
    x_min = 0.0 - df.index(geo_scale,0) * 0.5
    y_min = 0.0 - df.index(geo_scale,1) * 0.5
    z_min = 0.0 - df.index(geo_scale,2) * 0.5
    x_max = df.index(geo_scale,0) * 0.5
    y_max = df.index(geo_scale,1) * 0.5
    z_max = df.index(geo_scale,2) * 0.5
    x_clamped = df.clamp(x, x_min, x_max)
    y_clamped = df.clamp(y, y_min, y_max)
    z_clamped = df.clamp(z, z_min, z_max)

    # normalize to volume (to lie in [0,1])
    x_norm = (x_clamped / df.index(geo_scale,0)) + 0.5
    y_norm = (y_clamped / df.index(geo_scale,1)) + 0.5
    z_norm = (z_clamped / df.index(geo_scale,2)) + 0.5

    # convert to index space
    res_f = 256.0
    res_i = 256
    x_int = df.cast_int(x_norm * (res_f - 1.0))
    y_int = df.cast_int(y_norm * (res_f - 1.0))
    z_int = df.cast_int(z_norm * (res_f - 1.0))

    # if clamped, compute distance to volume box
    if (x <= x_min) or (x >= x_max) or (y <= y_min) or (y >= y_max) or (z <= z_min) or (z >= z_max):
        dist = df.load(geo_sdf, sub2ind(res_i,res_i,res_i, x_int    , y_int    , z_int))
        snap_dist = df.length(df.float3(x-x_clamped, y-y_clamped, z-z_clamped))
        return dist+snap_dist
    #return df.leaky_min(dist, dist+snap_dist, 1.0)

    x_int = df.max(df.min(x_int, (res_i - 2)), 0)
    y_int = df.max(df.min(y_int, (res_i - 2)), 0)
    z_int = df.max(df.min(z_int, (res_i - 2)), 0)
    #if tid == 99999999:

    # # compute fractional distance to next cell along each axis
    x_frac = (x_norm * (res_f - 1.0)) % 1.0
    y_frac = (y_norm * (res_f - 1.0)) % 1.0
    z_frac = (z_norm * (res_f - 1.0)) % 1.0

    a = df.load(geo_sdf, sub2ind(res_i,res_i,res_i, x_int    , y_int    , z_int))
    b = df.load(geo_sdf, sub2ind(res_i,res_i,res_i, x_int + 1, y_int    , z_int))
    c = df.load(geo_sdf, sub2ind(res_i,res_i,res_i, x_int    , y_int + 1, z_int))
    d = df.load(geo_sdf, sub2ind(res_i,res_i,res_i, x_int + 1, y_int + 1, z_int))
    e = df.load(geo_sdf, sub2ind(res_i,res_i,res_i, x_int    , y_int    , z_int + 1))
    f = df.load(geo_sdf, sub2ind(res_i,res_i,res_i, x_int + 1, y_int    , z_int + 1))
    g = df.load(geo_sdf, sub2ind(res_i,res_i,res_i, x_int    , y_int + 1, z_int + 1))
    h = df.load(geo_sdf, sub2ind(res_i,res_i,res_i, x_int + 1, y_int + 1, z_int + 1))
    ...
    # x axis
    a_to_b = lerp(a, b, x_frac)
    c_to_d = lerp(c, d, x_frac)
    e_to_f = lerp(e, f, x_frac)
    g_to_h = lerp(g, h, x_frac)

    # y axis
    y1 = lerp(a_to_b, c_to_d, y_frac)
    y2 = lerp(e_to_f, g_to_h, y_frac)
    ...
    if snap_dist > 0.0:
        ...
        #df.print(snap_dist)
        #df.print(geo_scale)
    return lerp(y1, y2, z_frac)
    #return snap_dist + lerp(y1, y2, z_frac)
...
# allows us to reuse the Cholesky decomposition from the forward pass
@df.kernel
def eval_dense_solve_batched(b_start: df.tensor(int), A_start: df.tensor(int), A_dim: df.tensor(int), A: df.tensor(float), L: df.tensor(float), b: df.tensor(float), x: df.tensor(float)):
...
class SemiImplicitIntegrator:
    ...
    def forward(self, model: Model, state_in: State, dt: float, update_mass_matrix=True) -> State:
        ...
        if dflex.config.no_grad:
            ...
            self._simulate(df.Tape(), model, state_in, state_in, dt, update_mass_matrix=update_mass_matrix)
            return state_in
                ...
                if (model.contact_count > 0):
                    ...
                    tape.launch(
                        ...
                        inputs=[
                            ...
                            model.contact_dist if len(model.geo_sdfs) == 0 else model.geo_sdfs[0],
                            model.contact_body0,
                            model.contact_body1,
                            model.contact_shape0,
...
# just for testing (see tests/test_cross_adj.py), todo: move this
@df.kernel
def _cross_kernel(u: df.tensor(df.float3),
                  v: df.tensor(df.float3),
                  # output
                  out: df.tensor(df.float3)):
    tid = df.tid()

    u_ = df.load(u,tid)
    v_ = df.load(v,tid)

    u_x_v = df.cross(u_,v_)

    if tid == 0:
        df.print(u_)
        df.print(v_)
        df.print(u_x_v)
    df.store(out, tid, u_x_v)

# just for testing (see tests/test_abs_adj.py), todo: move this
@df.kernel
def _abs_kernel(u: df.tensor(float),
                  # output
                  out: df.tensor(float)):
    tid = df.tid()
