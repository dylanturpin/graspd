...
contact_forces = torch.zeros_like(state.contact_f_s[box_contact_inds,:])

# initial particle
joint_d = base_joint_q.numel()
particle = torch.zeros(joint_d + contact_forces.numel(),device=device)
particle[:joint_d] = base_joint_q

box_contact_inds = torch.where(model.contact_body1 == 26)[0]
self_contact_inds = torch.where(model.contact_body1 != 26)[0]
contact_forces = torch.zeros_like(state.contact_f_s[box_contact_inds,:])
contact_d = contact_forces.numel()
#contact_forces[:,-2] = -0.01 / contact_forces.shape[0] / sim_dt * 21.6
...
def force_achieves_task():
    old_contact_ke = model.contact_ke
    old_contact_kf = model.contact_kf
    model.contact_ke = 0.0
    model.contact_kf = 0.0
    state = model.state()
    state.joint_act[-6:] = -contact_forces.sum(dim=0)
    ...
    model.contact_ke = old_contact_ke
    model.contact_kf = old_contact_kf
    return l
...
def loss(particle):
    ...
    contact_forces = particle[len(inds):].reshape(-1,6) * 100.0

    # SIMULATE WITH PRESCRIBED FORCES
    old_contact_ke = model.contact_ke
    old_contact_kf = model.contact_kf
    model.contact_ke = 0.0
    model.contact_kf = 0.0
    ...
    l = l_physics + l_self_collision + l_joint + l_task
    return l

es = cma.CMAEvolutionStrategy(particle.detach().cpu().numpy(), 0.01, {'popsize': 100, 'conditioncov_alleviate': [1e14,1e14],
    'BoundaryHandler': cma.s.ch.BoundTransform, 'bounds': [[-np.inf]*7 + model.joint_limit_lower[7:].tolist() + [-np.inf]*contact_d, [np.inf]*7 + model.joint_limit_upper[7:].tolist() + [np.inf]*contact_d]})
...
while not es.stop():
    ...
    solutions_[:,7:joint_d] = torch.clamp(solutions_[:,7:joint_d], model.joint_limit_lower[7:joint_d][None,:], model.joint_limit_upper[7:joint_d][None,:])
...
for best in bests:
    state = model.state()
    base_joint_q = torch.tensor(best[:joint_d],dtype=torch.float,device=device)
    base_joint_q[3:7] /= torch.norm(base_joint_q[3:7])
    base_joint_q[7:] = torch.clamp(base_joint_q[7:], model.joint_limit_lower[7:joint_d], model.joint_limit_upper[7:joint_d])
    state.joint_q[inds] = base_joint_q
