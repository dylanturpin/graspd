...
class GaussianMixture(torch.nn.Module):
    ...
    def _init_params(self):
        ...
        if self.covariance_type == "diag":
            if self.var_init is not None:
                ...
                self.var = torch.nn.Parameter(torch.ones((1, self.n_components, self.n_features),device="cuda:0"), requires_grad=False)
            ...
            if self.var_init is not None:
                ...
                self.var = torch.nn.Parameter(
                    torch.eye(self.n_features,dtype=torch.float32,device="cuda:0").reshape(1, 1, self.n_features, self.n_features).repeat(1, self.n_components, 1, 1),
                    requires_grad=False)

        # (1, k, 1)
        self.pi = torch.nn.Parameter(torch.zeros((1, self.n_components, 1),device="cuda:0"), requires_grad=False).fill_(1.0 / self.n_components)
    ...
    def sample(self, n):
        ...
        for k in range(self.n_components):
            if self.covariance_type == "diag":
                ...
                x_k = torch.tensor([],device=x.device)
                if counts[k] > 0.0:
    ...
    def _calculate_log_det(self, var):
        ...
        for k in range(self.n_components):
            ...
            #(L,info) = torch.linalg.cholesky_ex(var[0,k])
            #L = torch.linalg.cholesky(var[0,k])
            #if info == 0:
                #log_det[k] = 2 * torch.log(torch.diagonal(L)).sum()
                #log_det[k] = 2 * torch.log(torch.diagonal(L)).sum()
            #else:
                #log_det[k] = 2 * torch.log(torch.diagonal(var[0,k])).sum()
