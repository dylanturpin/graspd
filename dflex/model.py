...
class Model:
    ...
    def __init__(self, adapter):
        ...
        # COLLECT GRASP SETTINGS
        self.contact_distance = 0.1
        self.contact_ke = 1e+6 #1e+6
        self.contact_kd = 0.0
        self.contact_kf = 1e+8 #1e+8
        self.contact_mu = 0.8
        self.contact_alpha = 0.0
        #self.contact_mu = 0.2

        # EVAL GRASP SETTINGS
        #self.contact_distance = 0.1
        #self.contact_ke = 1e+8
        #self.contact_kd = 3e+4
        #self.contact_kf = 1e+10
        #self.contact_mu = 0.8

        # EVAL GRASP SETTINGS maybe latest?
        # self.contact_distance = 0.1
        # self.contact_ke = 1e+7
        # self.contact_kd = 1e+3
        # self.contact_kf = 1e+9
        # self.contact_mu = 0.2 # sometimes 0 better
    ...
    def collide(self, state: State):
        ...
        the model's set of contacts stored as the following attributes:
            ...
            * **contact_body1**: Tensor of ints with second rigid body index (or -1 to indicate ground)
            * **contact_shape0**: Tensor of ints with first shape index 
            * **contact_shape1**: Tensor of ints with second shape index (or -1 to indicate ground)
        ...
        def add_contact(b0, b1, s0, s1, t, p0, d, m):
            body0.append(b0)
            body1.append(b1)
            shape0.append(s0)
            shape1.append(s1)
        ...
        if self.ground:
            for i in range(self.shape_count):
                ...
                if (geo_type == GEO_SPHERE):
                    ...
                    add_contact(self.shape_body[i], -1, i, -1, X_bs, (0.0, 0.0, 0.0), radius, i)
                    ...
                    add_contact(self.shape_body[i], -1, i, -1, X_bs, (-half_width, 0.0, 0.0), radius, i)
                    add_contact(self.shape_body[i], -1, i, -1, X_bs, (half_width, 0.0, 0.0), radius, i)
                    ...
                    add_contact(self.shape_body[i], -1, i, -1, X_bs, (-edges[0], -edges[1], -edges[2]), 0.0, i)        
                    add_contact(self.shape_body[i], -1, i, -1, X_bs, ( edges[0], -edges[1], -edges[2]), 0.0, i)
                    add_contact(self.shape_body[i], -1, i, -1, X_bs, (-edges[0],  edges[1], -edges[2]), 0.0, i)
                    add_contact(self.shape_body[i], -1, i, -1, X_bs, (edges[0], edges[1], -edges[2]), 0.0, i)
                    add_contact(self.shape_body[i], -1, i, -1, X_bs, (-edges[0], -edges[1], edges[2]), 0.0, i)
                    ...
                    for v in mesh.vertices:
                        ...
                        add_contact(self.shape_body[i], -1, i, -1, X_bs, p, 0.0, i)

        # add all pairs
        for i in range(self.shape_count):
            for j in range(self.shape_count):
                # exclude self-collision (same shape or same body)
                if i == j: continue # exclude self collision
                if self.shape_body[i] == self.shape_body[j]: continue # exclude self collision

                # mesh can only be first member of collision pair (second treated as sdf)
                geo_type_i = self.shape_geo_type[i].item()
                geo_type_j = self.shape_geo_type[j].item()
                if geo_type_j == GEO_MESH: continue # mesh can only be first member of pair

                # exclude self-collision (same articulation)
                # get index of first item less than i, first item less than j
                #art_i = (self.articulation_joint_start >= self.shape_body[i]+1).nonzero(as_tuple=True)[0][0]
                #art_j = (self.articulation_joint_start >= self.shape_body[j]+1).nonzero(as_tuple=True)[0][0]
                #if art_i == art_j: continue

                # COLLECT GRASP SETTINGS
                if self.shape_body[i] == 6: continue

                # EVAL GRASP SETTINGS
                #if self.shape_body[j] != 26: continue
                #if self.shape_body[i] == 26: continue

                bodyi = self.shape_body[i]
                bodyj = self.shape_body[j]
                ...
                #     [0,11],
                #     [0,16],
                #     [0,17], # fist 2 thumb links can intersect hand
                #     [1,2],
                #     [2,3],
                #     [3,4],
                #     [4,5],
                ...
                for pair in rejected_pairs:
                    if bodyi in pair and bodyj in pair:
                        reject = True
                        break
                ...
                if (geo_type_i == GEO_SPHERE):

                    radius = self.shape_geo_scale[i][0].item()

                    add_contact(self.shape_body[i], self.shape_body[j], i, j, X_bs, (0.0, 0.0, 0.0), radius, i)

                elif (geo_type_i == GEO_CAPSULE):

                    radius = self.shape_geo_scale[i][0].item()
                    half_width = self.shape_geo_scale[i][1].item()

                    add_contact(self.shape_body[i], self.shape_body[j], i, j, X_bs, (-half_width, 0.0, 0.0), radius, i)
                    add_contact(self.shape_body[i], self.shape_body[j], i, j, X_bs, (half_width, 0.0, 0.0), radius, i)

                elif (geo_type_i == GEO_BOX):

                    edges = self.shape_geo_scale[i].tolist()

                    # add box mesh and primitive
                    box_pv = pv.Box(level=4, quads=False)
                    vertices = box_pv.points * np.array(edges)

                    for k in range(vertices.shape[0]):
                        add_contact(self.shape_body[i], self.shape_body[j], i, j, X_bs, vertices[k,:], 0.0, i)        

                    if not np.all(vertices[0,:] == (-np.array(edges))):
                        print("FAIL")

                    # add_contact(self.shape_body[i], self.shape_body[j], i, j, X_bs, (-edges[0], -edges[1], -edges[2]), 0.0, i)        
                    # add_contact(self.shape_body[i], self.shape_body[j], i, j, X_bs, ( edges[0], -edges[1], -edges[2]), 0.0, i)
                    # add_contact(self.shape_body[i], self.shape_body[j], i, j, X_bs, (-edges[0],  edges[1], -edges[2]), 0.0, i)
                    # add_contact(self.shape_body[i], self.shape_body[j], i, j, X_bs, (edges[0], edges[1], -edges[2]), 0.0, i)
                    # add_contact(self.shape_body[i], self.shape_body[j], i, j, X_bs, (-edges[0], -edges[1], edges[2]), 0.0, i)
                ...
                elif (geo_type_i == GEO_MESH):

                    mesh = self.shape_geo_src[i]
                    scale = self.shape_geo_scale[i].cpu()

                    for v in mesh.vertices:

                        p = (v[0] * scale[0], v[1] * scale[1], v[2] * scale[2])

                        add_contact(self.shape_body[i], self.shape_body[j], i, j, X_bs, p, 0.0, i)
...
class ModelBuilder:
    ...
    def add_shape_sdf(self,
                       ...
                       kf: float=1000.0,
                       mu: float=0.5):
        """Adds a discretized sdf collision shape to a link.
        ...
        Args:
            ...
            sdf: The discretized sdf, a list of floats, row major, length: res^3
        ...
        self.geo_sdfs = [sdf]
        self._add_shape(body, pos, rot, GEO_SDF, (scale[0], scale[1], scale[2]), sdf, density, ke, kd, kf, mu)
    ...
    def compute_box_inertia(self, density: float, w: float, h: float, d: float) -> tuple:
        ...
        Returns:
            ...
            A tuple of (mass, inertia) with inertia specified around the origin
        """
        return self.compute_sphere_inertia(density,w/2.0)
    ...
    def _compute_shape_mass(self, type, scale, src, density):
        ...
        if (type == GEO_SPHERE):
            ...
            return self.compute_sphere_inertia(density, 0.1)
            ...
            return self.compute_sphere_inertia(density, 0.05)
