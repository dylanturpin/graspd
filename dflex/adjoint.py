...
@builtin("stop_negative_grad")
class StopNegativeGradFunc:
    @staticmethod
    def value_type(args):
        return float
...
class Stmt:
    def __init__(self, cond, forward, forward_replay, reverse, ret_forward, ret_line):
        ...
        # all forward code outside of conditional branch *since last return*
        self.forward = forward
        self.forward_replay = forward_replay
        # all reverse code including the reverse of any code in ret_forward
        self.reverse = reverse

        # all forward commands in the return statement except the actual return statement
        self.ret_forward = ret_forward
...
class Adjoint:
    ...
    def add_bool_op(adj, op_string, exprs):
        output = adj.add_var(bool)
        command = "var_" + str(output) + " = " + (" " + op_string +
                                                  " ").join(["var_" + str(expr) for expr in exprs]) + ";"
    ...
    def add_call(adj, func, inputs, prefix='df::'):
        ...
        if (func.value_type(inputs) == None):
            ...
            if (len(inputs)):
                reverse_call = prefix + "{}({}, {});".format("adj_" + func.key, adj.format_args(
                    "var_", inputs), adj.format_args("adj_", inputs))
        ...
        # function (one output)
            ...
            forward_call = "var_{} = ".format(
                output) + prefix + "{}({});".format(func.key, adj.format_args("var_", inputs))
            adj.add_forward(forward_call)
    ...
    def add_return(adj, var):
        ...
        if (var == None):
            adj.add_forward("return;".format(
                var), "goto label{};".format(adj.label_count))
        else:
            adj.add_forward("return var_{};".format(
                var), "goto label{};".format(adj.label_count))
    ...
    def begin_for(adj, iter, start, end):
        ...
        adj.add_forward(
            "for (var_{0}=var_{1}; var_{0} < var_{2}; ++var_{0}) {{".format(iter, start, end), "if (false) {")
        adj.add_reverse("}")
    ...
    def end_for(adj, iter, start, end):
        ...
        adj.add_forward("}")
        adj.add_reverse(
            "for (var_{0}=var_{2}-1; var_{0} >= var_{1}; --var_{0}) {{".format(iter, start, end))
        ...
        try:
            ...
            if (isinstance(node, ast.FunctionDef)):
                ...
                if 'return' in adj.symbols and adj.symbols['return'] is not None:
                    ...
                    stmt = Stmt(None, adj.body_forward, adj.body_forward_replay, reversed(
                        adj.body_reverse), [], "")
                ...
                if len(node.orelse) != 0:
                    raise SyntaxError(
                        "Else statements not currently supported")
                ...
                for items in symbols_prev.items():
                    ...
                    if var1 != var2:
                        ...
                        out = adj.add_call(functions["select"], [
                                           cond, var1, var2])
                        adj.symbols[sym] = out
                ...
                out = adj.add_bool_op(
                    func, [adj.eval(expr) for expr in node.values])
                ...
                if (node.id in adj.symbols):
                    ...
                    raise KeyError(
                        "Referencing undefined symbol: " + str(node.id))
                ...
                if out is not None:        # set return type of function
                    ...
                    if adj.return_var is not None and adj.return_var.ctype() != return_var.ctype():
                        raise TypeError(
                            "error, function returned different types")
                ...
                print(
                    "[WARNING] ast node of type {} not supported".format(type(node)))
            ...
            print("Error: {} while transforming node {} in func: {} at line: {} col: {}: \n    {}".format(
                e, type(node), adj.func.__name__, node.lineno, node.col_offset, lines[max(node.lineno-1, 0)]))
            raise
...
def codegen_func_forward(adj, func_type='kernel', device='cpu'):
    ...
    for var in adj.variables:
        if var.constant == None:
            ...
            s += "    const " + var.ctype() + " var_" + str(var.label) + \
                " = " + str(var.constant) + ";\n"
...
def codegen_func(adj, device='cpu'):
    ...
    forward_body = codegen_func_forward(
        adj, func_type='function', device=device)
    reverse_body = codegen_func_reverse(
        adj, func_type='function', device=device)
...
def codegen_module_decl(adj, device='cpu'):
    ...
    s = template.format(name=adj.func.__name__, forward_args=indent(
        forward_args), reverse_args=indent(reverse_args))
    return s
...
def kernel(f):
    ...
    class Kernel:
        ...
        def register(self, module):
            ...
            self.forward_cpu = eval(
                "module." + self.func.__name__ + "_cpu_forward")
            self.backward_cpu = eval(
                "module." + self.func.__name__ + "_cpu_backward")
...
def filter_grads(grads):
    ...
    for g in grads:
        if torch.is_tensor(g) and (g.dim() == 0 or len(g) > 0):
            outputs.append(g)
        else:
            outputs.append(None)
    ...
    for arg in inputs:
        if isinstance(arg, torch.Tensor):
            if (arg.dtype == torch.float64) or (arg.dtype == torch.float16):
                raise TypeError("Tensor {arg} has invalid dtype {dtype}".format(
            ...
            if device == 'cpu':
                ...
                if arg.is_cuda:
                    raise TypeError(
                        "Tensor {arg} is using CUDA but was expected to be on the CPU.".format(arg=arg))
            elif device.startswith('cuda'):
                if not arg.is_cuda:
                    raise TypeError(
                        "Tensor {arg} is not on a CUDA device but was expected to be using CUDA.".format(arg=arg))
            else:
                raise ValueError("Device {} is not supported".format(device))
    ...
    if (check_grad == True and no_grad == False):
        try:
            torch.autograd.gradcheck(
                TorchFunc.apply, params, eps=1e-2, atol=1e-3, rtol=1.e-3, raise_exception=True)
        except Exception as e:
...
class Tape:
    ...
    def launch(self, func, dim, inputs, outputs, adapter, preserve_output=False, skip_check_grad=False):
        ...
        if (dim > 0):
            ...
            if dflex.config.check_grad == True and skip_check_grad == False:
                ...
                launch_torch(func, dim, inputs_copy, outputs_copy,
                             adapter, preserve_output, check_grad=True)
    ...
    def replay(self):
        ...
        for kernel in reversed(self.launches):
            ...
            if adapter == 'cpu':
                func.backward_cpu(
                    *[dim, *inputs, *outputs, *adj_inputs, *adj_outputs])
            elif adapter.startswith('cuda'):
                func.backward_cuda(
                    *[dim, *inputs, *outputs, *adj_inputs, *adj_outputs])
