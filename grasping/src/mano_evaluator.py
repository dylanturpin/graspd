...
class ManoEvaluator():
    ...
    def run(self, initial_guess):
        ...
        for i in range(n):
            ...
            #contact_forces.data[:] = box_contact_forces.detach()
            for i_inner in range(n_inner_contact_forces):
                # warmup learning rate
                #if (i*n_inner_contact_forces+ i_inner)<n_warmup:
                if i<n_warmup:
                    #for param_group in contact_forces_optimizer.param_groups:
                    param_group = contact_forces_optimizer.param_groups[0]
                    #param_group["lr"] = (i*n_inner_hand_q+i_inner) * (lr_contact_forces / n_warmup)
                ...
                l_physics_ = 0.0
                for j in range(self.n_vels):
                    #l_ = 1e-4* ((contact_forces[j,:,:] - box_contact_forces[j,:,:].detach())**2).sum()
                    #l_ = (((contact_forces[j,:,:] - box_contact_forces[j,:,:].detach()).abs()+1e-6)**0.5).sum()
                    ...
                    l_physics_ += 1e-2*l_
                #l_physics_ += 1e-4*(contact_forces**2).sum()
                ...
                #print(f"+{i_inner} {mdmm_return.fn_values[0]} {l_physics_}")
                #if i_inner > 200:
                    #if (l_task_old - mdmm_return.fn_values[0]).abs() < 1e-8 and (l_physics_-l_physics_old).abs() < 1e-8:
                        #break
            

            #hand_q = hand_q.detach()
            #hand_q.requires_grad_()
